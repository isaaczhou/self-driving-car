...
def dErrors(X, y, y_hat):
    DErrorsDx1 = error(y, y_hat) * X.T[0]
    DErrorsDx2 = error(y, y_hat) * X.T[1]
    DErrorsDb = error(y,y_hat) * X.T[2] # T[2] is 1
...
def gradientDescentStep(X, y, W, b, learn_rate = 0.01):
	y_hat = prediction(X,W,b) 	# TODO: Calculate the prediction

	gradient = dErrors(X, y, y_hat)     # TODO: Calculate the gradient

	for i in range(len(gradient)):
		W[0] += learn_rate*gradient.T[0]
		W[1] += learn_rate*gradient.T[1]
	e = error(y, y_hat)     # This calculates the error
	return W, b, e
#+END_SRC
